			...
			<div class="content">
				...
				<div class="container-fluid">
							...
							<div class="page-title-box">
								<div class="page-title-right">
									<ol class="breadcrumb m-0">
										...
										<li class="breadcrumb-item"><a href="/admin/apply/list">자격증 신청 현황</a></li>
										<li class="breadcrumb-item active">자격증 신청  상세보기</li>
									</ol>
								</div>
								<h4 class="page-title">자격증 신청 내역</h4>
							</div>
						</div>
					</div>
					<!-- end page title -->
					...
					<form name="frm" role="form">
						...
						<input type='hidden' name='oriApprover' value="${login.userName}">
						<input type='hidden' name='approver' value="${login.userName}">
								...
								<%--		<c:if test="${applyVO.applyState eq '승인' or  applyVO.applyState eq '거절'}">--%>
										...
										<c:if test="${applyVO.applyState eq '승인' or  applyVO.applyState eq '거절'}">
											<div class="form-row">
												<div class="form-group col-md-6">
													<label for="inputAddress2" class="col-form-label">결재자</label>
													<input type="text" name="approver" id="approver" class="form-control" value="${applyVO.approver}" disabled="disabled">
												</div>
												<div class="form-group col-md-6">
													<label for="inputAddress2" class="col-form-label">결재일자</label>
													<fmt:formatDate value="${applyVO.approvalDate}" type="date" pattern="yyyy-MM-dd (HH:mm:ss)" var="approvalDate" />
													<input type="text" name="approvalDate" id="approvalDate"
													class="form-control" value="${approvalDate}" disabled="disabled">
												</div>
											</div>
										</c:if>
<%-- 
	...
	<script>
		...
		$(document).ready(function() {
			...
			$(".btn-secondary").on("click", function() {
				formObj.attr("action", "/admin/apply/check?mapping=read");
				formObj.attr("method", "post");
				formObj.submit();
			});
			...
			$(".btn-primary").on("click", function() {
				...
				formObj.attr("action", "/admin/apply/list?page=${cri.page}&perPageNum=${cri.perPageNum}&searchType=${cri.searchType}&keyword=${cri.keyword}&applyNo=${applyVO.applyNo}");
				formObj.submit();
			});
